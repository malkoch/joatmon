    ...
    def run(self):
        try:
            ...
            if self.action[0] == 'current':
                resp = requests.get(base_url + 'current.json', params={'key': key, 'q': self.action[1]})
                ...
                resp = requests.get(base_url + 'forecast.json', params={'key': key, 'q': self.action[1], 'days': 3})
            elif self.action[0] == 'search':
                resp = requests.get(base_url + 'search.json', params={'key': key, 'q': self.action[1]})
            elif self.action[0] == 'history':
                resp = requests.get(base_url + 'history.json', params={'key': key, 'q': self.action[1], 'dt': datetime.now().isoformat()})
            else:
