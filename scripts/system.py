...
class Task(BaseTask):
    def __init__(self, api):
        ...
        if namespace.memory:
            self.action = ['memory', namespace.memory]
        elif namespace.cpu:
            self.action = ['cpu', namespace.cpu]
        elif namespace.disk:
            self.action = ['disk', namespace.disk]
        elif namespace.battery:
            self.action = ['battery', namespace.battery]
        elif namespace.process:
            self.action = ['process', namespace.process]
    ...
    def run(self):
        if self.action[0] == 'memory':
            ...
            self.api.output(f'Memory Total: {convert_size(psutil.virtual_memory().total)}, '
            ...
            self.api.output(f'CPU Total Usage: {psutil.cpu_percent(percpu=False)}, '
            ...
            for d in psutil.disk_partitions():
                self.api.output(f'Disk Device: {d.device}, '
                                f'Disk Mount: {d.mountpoint}, '
                                f'Disk File System Type: {d.fstype}')

                self.api.output(f'Disk Total: {convert_size(psutil.disk_usage(d.device).total)}, '
            ...
            self.api.output(f'Battery Percent: {psutil.sensors_battery().percent}, '
