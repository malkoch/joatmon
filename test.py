...
class LocalIntent(IntentAgent):
    ...
    def train_model(self):
        ...
        optimizer = Adam(params=list(self.model.parameters()), lr=.0001)
        # optimizer = RMSprop(params=list(self.model.parameters()), lr=.00005, weight_decay=1e-6)
...
intent = LocalIntent(intents)
intent.train_model()

for i in filter(lambda x: x['name'], intents):
    result = intent.request(i['name'])
    print(i['name'], result)
