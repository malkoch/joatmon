...
class MSSQLDatabase(DatabasePlugin):
    ...
    async def _update_collection(self, collection):
        ...
        for table, column, dtype in cursor.fetchall():
            doc_dtype = get_type(dict(doc_fields)[column].dtype)
            if dtype != doc_dtype:
                cursor.execute(f'alter table {collection.__collection__} alter column {column} type {doc_dtype}')
            ...
            # check data type
            # if they are different need to alter the table
